...
function applyEvent(event) {
	...
	switch (event.type) {
		case "pi.turn.start":
			...
			break;
		case "pi.tool.start":
			state.activeTools.set(event.toolCallId || `${event.toolName}:${state.eventCount}`, {
				name: event.toolName,
		...
		case "lens.formatter.run": {
			...
			const cur = state.formatters.get(key) || { name: key, runs: 0, changed: 0, failed: 0, files: new Set() };
			cur.runs++;
		...
		case "lens.runner.run": {
			...
			const cur = state.runners.get(key) || { id: key, runs: 0, diagnostics: 0, failed: 0, durationMs: 0, files: new Set(), status: "" };
			cur.runs++;
		...
		case "lens.lsp": {
			...
			const cur = state.lsps.get(key) || { serverId: event.serverId, root: event.root, starts: 0, status: "", failures: 0, source: "" };
...
function render() {
	const rows = [];
	rows.push(`${cyan("pi-lens dashboard")} ${dim(`events=${state.eventCount} turn=${state.turnIndex ?? "?"} uptime=${Math.round((Date.now() - state.startedAt) / 1000)}s`)}`);
	...
	for (const [, tool] of [...state.activeTools.entries()].slice(-8)) {
		rows.push(`  ${green("●")} ${tool.name} ${dim(`${Math.round((Date.now() - tool.startedAt) / 1000)}s`)} ${dim(JSON.stringify(tool.args || {})).slice(0, 100)}`);
	}
	rows.push("");

	rows.push(cyan("Configs identified"));
	const configs = [...state.configs.values()].filter((c) => c.type === "lens.config.formatter_selected").slice(-10);
	if (configs.length === 0) rows.push(dim("  none yet"));
	for (const c of configs) {
		rows.push(`  ${link(c.filePath, rel(c.filePath))} → ${c.formatter || "none"} ${dim(c.reason || "")}`);
	}
	rows.push("");

	rows.push(cyan("Formatters"));
	if (state.formatters.size === 0) rows.push(dim("  none yet"));
	for (const f of [...state.formatters.values()].sort((a, b) => b.runs - a.runs).slice(0, 12)) {
		rows.push(`  ${f.failed ? red(f.name) : green(f.name)} runs=${f.runs} changed=${f.changed} files=${f.files.size}`);
	}
	rows.push("");

	rows.push(cyan("Linters / runners"));
	if (state.runners.size === 0) rows.push(dim("  none yet"));
	for (const r of [...state.runners.values()].sort((a, b) => b.runs - a.runs).slice(0, 18)) {
		const name = r.failed ? red(r.id) : r.diagnostics ? yellow(r.id) : green(r.id);
		rows.push(`  ${name} runs=${r.runs} diags=${r.diagnostics} files=${r.files.size} total=${r.durationMs}ms`);
	}
	rows.push("");
	...
	for (const l of [...state.lsps.values()].slice(-12)) {
		const status = l.status === "spawn_success" ? green(l.status) : l.failures ? red(l.status) : yellow(l.status);
		rows.push(`  ${l.serverId} ${status} starts=${l.starts} ${dim(rel(l.root))}`);
	}
	rows.push("");

	rows.push(cyan("Touched files + diagnostics"));
	const files = [...state.files.values()].sort((a, b) => b.lastAt - a.lastAt).slice(0, 16);
	if (files.length === 0) rows.push(dim("  none yet"));
	for (const f of files) {
		const ds = [...f.diagnostics.values()];
		const marker = ds.some((d) => d.semantic === "blocking" || d.severity === "error") ? red("●") : ds.length ? yellow("▲") : green("✓");
		rows.push(`  ${marker} ${link(f.filePath, rel(f.filePath))} ${dim(`${ds.length} diagnostics`)}`);
		for (const d of ds.slice(0, 3)) {
			const loc = d.line ? `:${d.line}${d.column ? `:${d.column}` : ""}` : "";
			rows.push(`     ${dim(d.tool || "")} ${link(f.filePath, `${d.rule || "diagnostic"}${loc}`, d.line, d.column)} ${String(d.message || "").slice(0, 90)}`);
		}
...
readNew();
try {
	const watcher = fs.watch(path.dirname(logPath), { persistent: true }, (_event, name) => {
		if (!name || path.resolve(path.dirname(logPath), name.toString()) === path.resolve(logPath)) readNew();
	});
	watcher.on("error", () => {
